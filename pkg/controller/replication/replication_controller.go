...
func NewReplicationManager(podInformer coreinformers.PodInformer, rcInformer coreinformers.ReplicationControllerInformer, kubeClient clientset.Interface, burstReplicas int) *ReplicationManager {
	...
	// 添加rc事件处理handler方法，针对于不同的event调用不同方法
	...
	// 添加pod事件处理handler方法，针对于不同的event调用不同方法
...
func (rm *ReplicationManager) Run(workers int, stopCh <-chan struct{}) {
	...
	for i := 0; i < workers; i++ {
		go wait.Until(rm.worker, time.Second, stopCh)//每秒调用一次
	}
...
func (rm *ReplicationManager) syncReplicationController(key string) error {
	...
	// 根据 key 提取 namespace 和 name
	namespace, name, err := cache.SplitMetaNamespaceKey(key)
	if err != nil {
		return err
	}
	// 根据 namespace 和 name 得到 replication controller
	...
	// 从 cache storage 获取 controller 的 ControlleeExpecatations
	// 中的 add del timestamp 等
