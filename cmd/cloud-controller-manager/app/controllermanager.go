...
import (
	...
	kubectrlmgrapp "k8s.io/kubernetes/cmd/kube-controller-manager/app"
...
func startControllers(c *cloudcontrollerconfig.CompletedConfig, stop <-chan struct{}, cloud cloudprovider.Interface, controllers map[string]InitFunc) error {
	if cloud != nil {
	...
	for controllerName, initFn := range controllers {
		if !kubectrlmgrapp.IsControllerEnabled(controllerName, ControllersDisabledByDefault, c.ComponentConfig.Generic.Controllers...) {
			klog.Warningf("%q is disabled", controllerName)
			continue
		}

		klog.V(1).Infof("Starting %q", controllerName)
		_, started, err := initFn(c, cloud, stop)
		if err != nil {
...
func NewControllerInitializers() map[string]InitFunc {
	...
	controllers["cloudnode"] = startCloudNodeController
